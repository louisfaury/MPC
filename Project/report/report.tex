...
\vspace{15pt}
	...
	\section{Battery storage}
	{
		...
		\paragraph{} Figures (\ref{fig::battery_purchase}) and (\ref{fig::battery_charge}) respectively display the power purchased from the grid and the charging power / capacity of the battery, along with the variation of the electrical power price. As expected, when internal gains, solar exposition, etc. allow it, most of the power is purchased when the price is low. Indeed, unlike when we did not consider this electricity buffer, some power was still needed during the day, when now almost none is bought at this period (this assertion becomes false when the battery storage is not high to prevent temperature drops in the day - starting near hour 130). 
		...
		\paragraph{} Ultimately, we see that increasing the battery storage capacity leads to bigger economies due to the lesser necessity to pay expensive electricity. When designing the heating system for a building, one should therefore find the good compromise: a high capacity battery with low loss is expensive to buy and maintain but will tend to decrease the energy bill of the building.
