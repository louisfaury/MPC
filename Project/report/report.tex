...
\vspace{15pt}
	...
	\section{First MPC Controller}
	{
		...
		\paragraph{} Because the controller is now able to consider the temperature raise due to the orientation of the room and the sun exposition, it allows a small undershoot by setting the corresponding input to 0 well before this event. Therefore, a smaller overshoot is achieved when increasing the horizon, since the room slightly cooled down before being being exposed to the sun. 
	}
	...
	\section{Night Setbacks}
	{
		...
		\left\{
			\begin{aligned}
				&y_{max} = 30° & \quad& \text{between 12pm to 0am, and 6pm to 12pm} \\
				&y_{max} = 26° &\quad & \text{between 8am to 6pm} \\
				&y_{min} = 18° &\quad & \text{between 12pm to 0am, and 6pm to 12pm} \\
				&y_{min} = 22° & \quad& \text{between 8am to 6pm} \\
			\end{aligned}\right.
		\end{equation}
